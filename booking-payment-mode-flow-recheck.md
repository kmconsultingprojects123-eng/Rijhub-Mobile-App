...
Mobile can read the current platform commission with:
...
Mobile can also pass an amount to preview the deduction:
...
The app should treat this as display data only. The backend still recalculates the commission during verified payment, so the client cannot control the actual deducted amount.
