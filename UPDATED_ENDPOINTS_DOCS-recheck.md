...
### Automated Commission and Payout Flow (Current):
1. Admin sets `COMPANY_FEE_PCT`.
2. Client pays the full service price.
3. Paystack webhook or `POST /payment/verify` confirms the payment.
4. Backend calculates `companyFee` from the full amount.
5. Backend stores the artisan net amount as `transferAmount`.
6. Backend transfers the net amount to the artisan when `PAYSTACK_AUTO_PAYOUT=true` and payout details exist.
7. If transfer cannot be started, backend credits the artisan internal wallet.
8. Artisan wallet stats and customer spend stats are updated once per transaction.

### Current Commission Endpoint:
Mobile can fetch the current company commission percentage:
...
```http
GET /payment/commission?amount=5000
Authorization: Bearer <token>
```
...
### Mobile Complete Button for `afterCompletion`:
Treat Complete as Pay & Complete:
1. User taps Complete.
2. App calls `POST /booking/:id/pay-after-completion`.
3. App opens Paystack checkout using `payment.authorization_url`.
4. App calls `POST /payment/verify` with the Paystack `reference`.
5. Backend automatically deducts `companyFee` and pays/credits `transferAmount` to the artisan.
...
Do not call `POST /booking/:id/complete` first for `afterCompletion` bookings. The backend rejects completion until payment is verified and payout has been released/paid.
