...
class _SplashScreenPage2WidgetState extends State<SplashScreenPage2Widget> with TickerProviderStateMixin {
  ...
  void initState() {
    ...
    _bgFade = CurvedAnimation(parent: _bgFadeController!, curve: Curves.easeInOut);
    ...
    return Container(
      ...
      child: ElevatedButton(
        onPressed: () async {
          ...
          // ignore: avoid_print
          if (kDebugMode) debugPrint('Navigating to CreateAccount2 with role: $role');
          ...
          NavigationUtils.safePushNoAuth(context, CreateAccount2Widget(initialRole: role));
        },
                              ...
                              onPressed: () async {
                                ...
                                try {
                                  ...
                                  if (res['success'] == true) {
                                    ...
                                    if (body is Map) {
                                      token = (body['token'] ?? body['accessToken'] ?? body['data']?['token'] ?? body['data']?['accessToken'] ?? body['user']?['token'])?.toString();
                                      // Try to find a user object in common locations
                                      if (body['user'] is Map) userProfile = Map<String, dynamic>.from(body['user']);
                                      else if (body['data'] is Map && body['data']['user'] is Map) userProfile = Map<String, dynamic>.from(body['data']['user']);
                                      else if (body['data'] is Map && (body['data']['profile'] is Map)) userProfile = Map<String, dynamic>.from(body['data']['profile']);
                                      else {
                                        // If body itself contains common profile fields (id/name/email), use it as profile
                                        final hasId = body.containsKey('_id') || body.containsKey('id') || body.containsKey('userId');
                                        if (hasId) userProfile = Map<String, dynamic>.from(body);
                                      }
                                    }

                                    if (token != null && token.isNotEmpty) {
                                      // Persist token and role explicitly
                                      try { await TokenStorage.saveToken(token); } catch (_) {}
                                      try { await TokenStorage.saveRole('guest'); } catch (_) {}

                                      // Update in-memory auth state: AuthNotifier and AppStateNotifier
                                      try { await AuthNotifier.instance.setGuest(token: token); } catch (_) {}
                                      ...
                                      final msg = 'Guest login failed: server did not return a token.';
                                      if (!mounted) return;
                                      await showAppErrorDialog(context, title: 'Error', desc: msg);
                                    }
                                  } else {
                                    final err = res['error'];
                                  ...
                                  if (!mounted) return;
                                  try { Navigator.of(context, rootNavigator: true).pop(); } catch (_) { try { NavigationUtils.safeMaybePop(context); } catch (_) {} }
                              ...
                              TextButton(
                                onPressed: () {
                                  try {
                                    ...
                                    try { Navigator.of(context).push(MaterialPageRoute(builder: (_) => const LoginAccountWidget())); } catch (_) {}
                                  }
                                },
                                child: const Text(
                                  'Login',
