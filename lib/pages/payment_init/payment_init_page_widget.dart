...
class PaymentInitPageWidget extends StatefulWidget {
  ...
  const PaymentInitPageWidget(
      {super.key, required this.payment, this.booking, this.quote});
...
class _PaymentInitPageWidgetState extends State<PaymentInitPageWidget> {
  ...
  void initState() {
    ...
    if (kDebugMode) {
      try {
        debugPrint(
            'PaymentInitPageWidget received payment: ${jsonEncode(widget.payment)}');
      } catch (_) {}
      try {
        debugPrint(
            'PaymentInitPageWidget received booking: ${jsonEncode(widget.booking)}');
      } catch (_) {}
    }
    _resolveInitialData();
    _populateEmail();
  ...
  void _resolveInitialData() {
    ...
    _reference = _findKey(
            payment, ['reference', 'ref', 'tx_ref', 'transaction_reference'])
        ?.toString();
    ...
    try {
      if (widget.quote != null) {
        ...
        amount ??= _findKey(
            q, ['total', 'amount', 'price', 'quoteTotal', 'proposedPrice']);
      }
    } catch (_) {
      amount = null;
    }
    // fallback to payment node if quote didn't provide amount
    amount ??= _findKey(payment,
            ['total', 'amount', 'price', 'quoteTotal', 'proposedPrice']) ??
        payment['amount'] ??
    ...
    try {
      ...
      if (meta is Map && meta['clientTotal'] != null) {
        ...
        if (ct is String)
          ct = num.tryParse(ct.replaceAll(RegExp(r'[^0-9.-]'), '')) ?? ct;
    ...
    // Fallback: recover bookingId from payment metadata (embedded by artisan detail page or
    // returned inside Paystack transaction metadata). This ensures we never try to create
    // a duplicate booking when the booking was already created before payment initiation.
    if (_bookingId == null || _bookingId!.isEmpty) {
      try {
        final meta = payment['metadata'];
        if (meta is Map) {
          final mid = meta['bookingId']?.toString();
          if (mid != null && mid.isNotEmpty) _bookingId = mid;
        }
      } catch (_) {}
    }
    ...
    if (kDebugMode)
      debugPrint(
          'PaymentInit(init): amount=$_amount authUrl=$_authUrl ref=$_reference bookingId=${_bookingId ?? '<null>'} blocking=${_blocking}');
  }
    ...
    try {
      ...
      if (q is Map) {
        ...
        final candidates = [
          ...
          mq['userId'],
          mq['ownerId'],
          mq['artisan_user'] ?? mq['artisanUser'] ?? mq['artisan']
        ];
        for (final c in candidates) {
          if (c == null) continue;
          if (c is Map) {
      ...
      if (_initRequestPayload != null) {
        final cand = _initRequestPayload!['artisanId'] ??
            (_initRequestPayload!['metadata'] is Map
                ? _initRequestPayload!['metadata']['artisanId']
                : null);
  ...
  String _displayAmount(dynamic a) {
    try {
      ...
      final numVal =
          num.tryParse(a.toString().replaceAll(RegExp(r'[^0-9.-]'), ''));
      if (numVal != null)
  ...
  Future<void> _initPaymentThenStart() async {
    ...
    if (needsSignInForAction()) {
      await showGuestAuthRequiredDialog(context,
          message:
              'You need to sign in or create an account to make payments.');
      return;
    }
    setState(() {
    ...
    if (_email == null || _email!.trim().isEmpty) {
      AppNotification.showError(context,
          'An email address is required to make payments. Please update your profile.');
      setState(() {
    ...
    if (_isQuotePayment()) {
      ...
      if (!accepted) {
        ...
        if (choice == 'signin') {
          ...
          if (!accepted2) {
            AppNotification.showError(context,
                'Could not accept quote after sign in. Payment aborted.');
            setState(() {
              _state = _PaymentState.checkout;
              _loading = false;
              _blocking = false;
          ...
          AppNotification.showInfo(
              context, 'Payment aborted — quote acceptance required');
          setState(() {
            _state = _PaymentState.checkout;
            _loading = false;
            _blocking = false;
    ...
    try {
      _initRequestPayload = Map<String, dynamic>.from(bodyMap);
    } catch (_) {
      _initRequestPayload = null;
    }
    ...
    try {
      ...
      try {
        if (qid != null && qid.toString().isNotEmpty) {
          final meta = bodyMap['metadata'] is Map
              ? Map<String, dynamic>.from(bodyMap['metadata'])
              : <String, dynamic>{};
    ...
    try {
      ...
      if (_bookingId != null && _bookingId!.isNotEmpty) {
        final uri = Uri.parse(
            '${_normalizeBaseUrl(API_BASE_URL)}/api/bookings/${_bookingId!}/pay-with-quote');
        if (kDebugMode)
          debugPrint(
              'Initializing payment via booking-scoped endpoint -> $uri');
        resp = await http
            .post(uri, headers: headers, body: jsonEncode(bodyMap))
            .timeout(const Duration(seconds: 10));
      } else {
        final uri = Uri.parse(
            '${_normalizeBaseUrl(API_BASE_URL)}/api/payments/initialize');
        if (kDebugMode)
          debugPrint('Initializing payment via generic endpoint -> $uri');
        resp = await http
            .post(uri, headers: headers, body: jsonEncode(bodyMap))
            .timeout(const Duration(seconds: 10));
      }
      if (kDebugMode)
        debugPrint('Payment init response -> ${resp.statusCode} ${resp.body}');
      if (resp.statusCode >= 200 && resp.statusCode < 300) {
        ...
        try {
          if (_initRequestPayload == null)
            _initRequestPayload = <String, dynamic>{};
          if (data is Map) {
        ...
        try {
          ...
          if (ebody is Map)
            serverMsg = ebody['message']?.toString() ??
                ebody['error']?.toString() ??
                '';
        } catch (_) {}
        ...
        if (resp.statusCode == 401) {
          ...
          final choice = await showDialog<String?>(
            ...
            builder: (ctx) => AlertDialog(
              ...
              content: const Text(
                  'You need to be signed in to make payments. Would you like to sign in now?'),
              actions: [
                TextButton(
                    onPressed: () => Navigator.of(ctx).pop('cancel'),
                    child: const Text('Cancel')),
          ...
          if (choice == 'signin') {
            try {
              await NavigationUtils.safePushNoAuth(
                  context, LoginAccountWidget());
            } catch (_) {
              try {
                await NavigationUtils.safePush(context, LoginAccountWidget());
              } catch (_) {}
          ...
          AppNotification.showError(
              context, 'Session required — sign in to continue');
        } else {
          AppNotification.showError(
              context,
  ...
  Future<dynamic> _startInAppPayment() async {
    ...
    try {
      FlowGuard.setPaymentActive(true);
    } catch (_) {}
    setState(() {
    ...
    try {
      ...
      final page = PaymentWebviewPageWidget(
          url: _authUrl!, successUrlContains: _reference);
      ...
      // Give the WebView GPU process (Chrome_InProcGp) time to finish shutting
      // down its OpenGL context before Flutter renders any new dialogs.
      // Without this delay the GPU thread is still mid-cleanup when Flutter
      // draws the next frame, causing a fatal SIGTRAP in libwebviewchromium.so.
      await Future.delayed(const Duration(milliseconds: 400));

      if (!success) {
        // Clear payment guard before returning.
        try {
          FlowGuard.setPaymentActive(false);
        } catch (_) {}
        ...
        if (mounted) {
          ...
          final jobTitle = widget.booking?['service'] ??
              widget.quote?['title'] ??
              widget.payment['service']?.toString();
          final bookingPrice = (_amount ??
                  widget.booking?['price'] ??
                  widget.quote?['amount'] ??
                  widget.payment['amount'])
              ?.toString();
          final bookingDateTime = widget.booking?['schedule'] ??
              widget.quote?['schedule'] ??
              widget.payment['schedule']?.toString();

          try {
            await _showBookingFailedBottomSheet(jobTitle?.toString(),
                bookingPrice?.toString(), bookingDateTime?.toString());
          } catch (e) {
            if (kDebugMode)
              debugPrint('Error showing booking failed bottom sheet: $e');
            AppNotification.showInfo(
                context, 'Payment cancelled — you can retry');
          }
        } else {
      ...
      if (returnedUrl != null && returnedUrl.isNotEmpty) {
        try {
          ...
          if (uri != null) {
            ...
            if ((_reference == null || _reference!.isEmpty) &&
                (q['reference'] ?? q['ref'] ?? q['tx_ref']) != null) {
              _reference =
                  (q['reference'] ?? q['ref'] ?? q['tx_ref'])?.toString();
            }
          }
        } catch (_) {}
      }
      ...
      try {
        ...
        if (kDebugMode)
          debugPrint('Immediate verification result -> $verified');
      } catch (e) {
      ...
      if (!verified) {
        ...
        if (mounted) {
          ...
          try {
            ...
            if (kDebugMode)
              debugPrint(
                  'Error showing booking failed bottom sheet after verification failure: $e');
            AppNotification.showInfo(
                context, 'Payment not verified — you can retry');
          }
        } else {
          try {
            AppNotification.showInfo(
      ...
      if (_bookingId == null || _bookingId!.isEmpty) {
        ...
        try {
          ...
          if (_isQuotePayment()) {
            String? qid;
            try {
              qid = widget.quote?['_id']?.toString() ??
                  widget.payment['quoteId']?.toString() ??
                  widget.payment['quote']?['_id']?.toString();
            } catch (_) {
              qid = null;
            }
            // Use stored _quoteId if available
            qid ??= _quoteId;
            final polled = await _pollForBookingFromQuote(qid,
                maxAttempts: 6, initialDelay: const Duration(seconds: 1));
            if (polled != null) {
              _bookingId = polled['bookingId'];
              threadIdFromCreation = polled['threadId'];
              if (kDebugMode)
                debugPrint(
                    'Polled booking from quote -> bookingId=$_bookingId threadId=$threadIdFromCreation');
            } else {
              if (kDebugMode)
                debugPrint(
                    'Polling for server-created booking returned null for quoteId=$qid');
            ...
            if (created != null) {
              ...
              if (kDebugMode)
                debugPrint(
                    'Booking created in _startInAppPayment after verification: bookingId=$_bookingId threadId=$threadIdFromCreation');
            } else {
              if (kDebugMode)
                debugPrint(
                    'Booking creation returned null even after verification');
            }
          }
        } finally {
          try {
      ...
      if (_isQuotePayment() && _bookingId != null && _bookingId!.isNotEmpty) {
        try {
          ...
          if (kDebugMode)
            debugPrint(
                'Accept-after-payment attempt -> $acceptedAfterPayment for bookingId=$_bookingId');
          if (acceptedAfterPayment) {
            try {
              final latestBooking = await _pollBookingStatus(_bookingId!,
                  maxAttempts: 6, initialDelay: const Duration(seconds: 2));
              if (latestBooking != null) {
                final serverTid =
                    (latestBooking['threadId'] ?? latestBooking['chat']?['_id'])
                        ?.toString();
          ...
          if (kDebugMode) debugPrint('Accept-after-payment error: $e');
        }
      }
      ...
      if (_bookingId != null && _bookingId!.isNotEmpty) {
        try {
          ...
          if (_isQuotePayment()) {
            ...
            if (kDebugMode)
              debugPrint(
                  'Quote flow: not calling protected confirm endpoint from client; polling for server webhook to mark payment as paid for bookingId=$_bookingId');
            ...
            try {
              ...
              if (mounted) {
                AppNotification.showSuccess(
                    context, 'Payment verified and booking created.');
                try {
            ...
            try {
              ...
              try {
                latestBooking = await _pollBookingStatus(_bookingId!,
                    maxAttempts: 6, initialDelay: const Duration(seconds: 2));
              } catch (_) {
                latestBooking = null;
              }
              ...
              if (mounted) {
                AppNotification.showSuccess(context,
                    'Payment verified and payment is held in escrow. Booking created.');
              ...
              try {
                if (latestBooking != null) {
                  final serverTid = (latestBooking['threadId'] ??
                          latestBooking['chat']?['_id'])
                      ?.toString();
                  if (serverTid != null && serverTid.isNotEmpty) {
                    threadIdFromCreation = serverTid;
                  }
              ...
              if (kDebugMode)
                debugPrint('Immediate confirm booking payment failed: $e');
              if (mounted)
                AppNotification.showInfo(
                    context,
                    _isQuotePayment()
                        ? 'Payment verified and booking created.'
                        : 'Payment verified awaiting artisan response.');
      ...
      if ((_bookingId != null && _bookingId!.isNotEmpty) &&
          (threadIdFromCreation == null || threadIdFromCreation!.isEmpty)) {
        try {
          final fetchedTid = await _fetchThreadIdForBooking(_bookingId!);
          if (kDebugMode)
            debugPrint(
                'fetchThreadIdForBooking -> bookingId=${_bookingId} fetchedThreadId=${fetchedTid ?? '<null>'}');
          if (fetchedTid != null && fetchedTid.isNotEmpty)
            threadIdFromCreation = fetchedTid;
        } catch (e) {
      ...
      AppNotification.showError(context,
          'Could not open in-app payment. Opening browser as fallback.');
      try {
  ...
  Future<void> _startBackgroundVerification(String? bookingId) async {
    ...
    for (var attempt = 1; attempt <= maxAttempts && !verified; attempt++) {
      if (kDebugMode)
        debugPrint('Background verify attempt $attempt/$maxAttempts');
      try {
        verified = await _verifyPayment();
      } catch (e) {
      ...
      if (!verified && attempt < maxAttempts) {
        ...
        if (kDebugMode)
          debugPrint('Background verification failed, retrying in ${waitMs}ms');
    ...
    if (verified) {
      ...
      if (bookingId != null && bookingId.isNotEmpty) {
        ...
        if (_isQuotePayment()) {
          if (kDebugMode)
            debugPrint(
                'Background verification: quote flow detected, skipping confirm for bookingId=$bookingId');
          if (mounted) {
            AppNotification.showSuccess(
                context, 'Payment verified and booking created (quote).');
            try {
              final cnt = await NotificationService.fetchUnreadCount();
              try {
                app_state_notifier.AppStateNotifier.instance
          ...
          try {
            ...
            if (mounted) {
              AppNotification.showSuccess(
                  context, 'Payment verified for your booking');
              try {
                final cnt = await NotificationService.fetchUnreadCount();
                try {
                  app_state_notifier.AppStateNotifier.instance
            ...
            if (_isQuotePayment() ||
                emsg.contains('404') ||
                emsg.toLowerCase().contains('no pending')) {
              if (kDebugMode)
                debugPrint(
                    'Confirm returned 404/no-pending but treating as success for bookingId=$bookingId');
              if (mounted)
                AppNotification.showSuccess(
                    context, 'Payment verified and booking is created.');
              try {
                final cnt = await NotificationService.fetchUnreadCount();
                try {
                  app_state_notifier.AppStateNotifier.instance
              ...
              if (mounted) {
                AppNotification.showInfo(context,
                    'Payment verified but could not update booking status automatically.');
                try {
                  final cnt = await NotificationService.fetchUnreadCount();
                  try {
                    app_state_notifier.AppStateNotifier.instance
      ...
      if (mounted)
        AppNotification.showInfo(context,
            'Payment verification is taking longer than expected. We will retry in the background.');
    }
  }
  ...
  Future<void> _confirmBookingPayment(String bookingId) async {
    ...
    if (token != null && token.isNotEmpty)
      headers['Authorization'] = 'Bearer $token';
    final uri = Uri.parse(
        '${_normalizeBaseUrl(API_BASE_URL)}/api/bookings/$bookingId/confirm-payment');
    final payload = <String, dynamic>{};
    if (_reference != null && _reference!.isNotEmpty)
      payload['reference'] = _reference;
    if (_lastVerifyResponse != null && _lastVerifyResponse!.isNotEmpty)
      payload['verifyResponse'] = _lastVerifyResponse;
    final resp = await http
        .post(uri, headers: headers, body: jsonEncode(payload))
        .timeout(_paymentRequestTimeout);
    if (kDebugMode)
      debugPrint('Confirm booking payment -> ${resp.statusCode} ${resp.body}');
    if (!(resp.statusCode >= 200 && resp.statusCode < 300)) {
    ...
    try {
      final result = await showDialog<dynamic>(
        ...
        builder: (ctx) => Dialog(
            ...
            child: SizedBox(
                width: double.infinity, height: double.infinity, child: page)),
      );
      return result;
    } catch (e) {
  ...
  Future<bool> _verifyPayment() async {
    try {
      ...
      if (token != null && token.isNotEmpty)
        headers['Authorization'] = 'Bearer $token';
      final uri =
          Uri.parse('${_normalizeBaseUrl(API_BASE_URL)}/api/payments/verify');
      ...
      final possibleRefs = <String?>[
        ...
        widget.payment['authorization'] is Map
            ? (widget.payment['authorization']['reference']?.toString() ??
                widget.payment['authorization']['tx_ref']?.toString())
            : null,
      ];
      ...
      if ((possibleRefs.where((e) => e != null && e.isNotEmpty).isEmpty) &&
          _authUrl != null &&
          _authUrl!.isNotEmpty) {
        try {
          final u = Uri.tryParse(_authUrl!);
      ...
      refCandidate = possibleRefs
          .firstWhere((e) => e != null && e.toString().trim().isNotEmpty,
              orElse: () => null)
          ?.toString();

      String? paymentIdCandidate = (widget.payment['_id'] ??
      ...
      if (refCandidate != null && refCandidate.isNotEmpty)
        body['reference'] = refCandidate;
      if (paymentIdCandidate != null && paymentIdCandidate.isNotEmpty)
        body['paymentId'] = paymentIdCandidate;
      ...
      final attemptVerify = (Map<String, dynamic> payload) async {
        try {
          final resp = await http
              ...
              .timeout(_paymentRequestTimeout);
          if (kDebugMode)
            debugPrint('Payment verify -> ${resp.statusCode} ${resp.body}');
          // capture last response for debugging
          try {
            _lastVerifyResponse = resp.body;
          } catch (_) {
              ...
              if (data is Map) {
                ...
                try {
                  ...
                  if (paymentNode is Map) {
                    final pStatus = (paymentNode['status'] ??
                        paymentNode['paid'] ??
                        paymentNode['isPaid'] ??
      ...
      if (!ok) {
        final altRef = (widget.payment['authorization'] is Map)
            ? (widget.payment['authorization']['reference']?.toString() ??
                widget.payment['authorization']['tx_ref']?.toString())
            : null;
        if (altRef != null && altRef.isNotEmpty && altRef != refCandidate) {
          ...
          if (kDebugMode)
            debugPrint('Retrying verification with altRef=$altRef');
  ...
  Future<Map<String, String?>?> _createBookingAfterPayment() async {
    try {
      ...
      try {
        ...
        try {
          bookingSource = (widget.payment['bookingSource'] ??
                      widget.payment['metadata'] is Map
                  ? widget.payment['metadata']['bookingSource']
                  : null)
              ?.toString();
        } catch (_) {
          bookingSource = null;
        ...
        if ((qid != null && qid.isNotEmpty) ||
            (bookingSource != null &&
                bookingSource.toString().toLowerCase() == 'quote')) {
        ...
        try {
          final cand = _initRequestPayload!['artisanId'] ??
              (_initRequestPayload!['metadata'] is Map
                  ? _initRequestPayload!['metadata']['artisanId']
                  : null);
          if (cand != null && cand.toString().isNotEmpty)
            artisanId = cand.toString();
        } catch (_) {}
      }
      ...
      if (kDebugMode)
        debugPrint('Create booking after payment -> status=${resp.statusCode}');
      if (resp.statusCode >= 200 &&
          resp.statusCode < 300 &&
          resp.body.isNotEmpty) {
        try {
          ...
          if (data is Map) {
            ...
            if (id != null) {
              ...
              try {
                if (kDebugMode)
                  debugPrint(
                      'createBookingAfterPayment -> server returned bookingId=$id threadId=${threadId ?? '<null>'}');
              } catch (_) {}
              return {'bookingId': id, 'threadId': threadId};
            }
          }
        } catch (e) {
          if (kDebugMode) debugPrint('Create booking parse error: $e');
        }
      } else {
        // If 400 about artisanId and we have acceptedQuote, try to fetch quote to get artisanId then retry
        if (resp.statusCode == 400 &&
            (resp.body?.toString().toLowerCase().contains('artisanid') ??
                false) &&
            body['acceptedQuote'] != null &&
            (body['artisanId'] == null ||
                body['artisanId'].toString().isEmpty)) {
          try {
            final qid = body['acceptedQuote'].toString();
            final fetched = await _fetchQuoteById(qid, headers);
            if (fetched != null) {
              final possible = (fetched['artisanId'] ??
                  fetched['artisan'] ??
                  fetched['artisanUser'] ??
                  fetched['artisan_user']);
              if (possible != null)
                body['artisanId'] = (possible is Map
                        ? (possible['_id'] ?? possible['id'])
                        : possible)
                    .toString();
              if (kDebugMode)
                debugPrint(
                    'Retrying create booking with artisanId=${body['artisanId']}');
              resp = await http
                  .post(uri, headers: headers, body: jsonEncode(body))
                  .timeout(const Duration(seconds: 12));
              if (kDebugMode)
                debugPrint(
                    'Create booking retry -> status=${resp.statusCode}, body=${resp.body}');
              if (resp.statusCode >= 200 &&
                  resp.statusCode < 300 &&
                  resp.body.isNotEmpty) {
                try {
                  ...
                  if (kDebugMode)
                    debugPrint('Create booking retry parse error: $e');
                }
              }
            }
          } catch (e) {
            if (kDebugMode) debugPrint('Retry create booking error: $e');
          }
        }
  ...
  Future<Map<String, String?>?> _pollForBookingFromQuote(String? qid,
      {int maxAttempts = 6,
    ...
    try {
      ...
      while (attempt < maxAttempts) {
        ...
        try {
          final uri = Uri.parse(
              '${_normalizeBaseUrl(API_BASE_URL)}/api/bookings?page=1&limit=50');
          final resp = await http
              .get(uri, headers: headers)
              .timeout(const Duration(seconds: 8));
          if (kDebugMode)
            debugPrint(
                'Poll bookings attempt $attempt for qid=$qid -> ${resp.statusCode} ${resp.body}');
          if (resp.statusCode >= 200 &&
              resp.statusCode < 300 &&
              resp.body.isNotEmpty) {
            final decoded = jsonDecode(resp.body);
            ...
            for (final it in items) {
              try {
                if (it is Map) {
                  final accepted = (it['acceptedQuote'] ??
                          it['accepted_quote'] ??
                          it['booking']?['acceptedQuote'] ??
                          it['booking']?['accepted_quote'])
                      ?.toString();
                  final paymentStatus =
                      (it['paymentStatus'] ?? it['booking']?['paymentStatus'])
                          ?.toString()
                          .toLowerCase();
                  if (accepted != null &&
                      accepted.isNotEmpty &&
                    ...
                    final id = (it['_id']?.toString() ??
                        it['id']?.toString() ??
                        (it['booking'] is Map
                            ? (it['booking']['_id']?.toString() ??
                                it['booking']['id']?.toString())
                            : null));
                    final tid = (it['threadId']?.toString() ??
                        it['chat']?['_id']?.toString());
                    if (id != null && id.isNotEmpty)
                      return {'bookingId': id, 'threadId': tid};
                  }
                  if (paymentStatus == 'paid') {
                    final accepted2 = (it['acceptedQuote'] ??
                            it['accepted_quote'] ??
                            it['booking']?['acceptedQuote'] ??
                            it['booking']?['accepted_quote'])
                        ?.toString();
                    if (accepted2 != null &&
                        accepted2.isNotEmpty &&
                        accepted2 == qid) {
                      final id = (it['_id']?.toString() ??
                          it['id']?.toString() ??
                          (it['booking'] is Map
  ...
  Future<Map<String, String?>?> _pollBookingsForQuote(String? qid,
      {int maxAttempts = 6,
      Duration initialDelay = const Duration(seconds: 1)}) async {
    return await _pollForBookingFromQuote(qid,
        maxAttempts: maxAttempts, initialDelay: initialDelay);
  }

  Future<void> _showAwaitingServerConfirmationBottomSheet(
    ...
    try {
          ...
          return SafeArea(
            child: Container(
              ...
              child: Column(mainAxisSize: MainAxisSize.min, children: [
                ...
                Text(
                    'We received your payment but are still waiting for the server to create the booking. This should complete shortly.',
                    textAlign: TextAlign.center,
                    style: FlutterFlowTheme.of(ctx).bodySmall),
                  ...
                  Expanded(
                      child: ElevatedButton(
                          onPressed: () async {
                            ...
                            try {
                              final polled = await _pollBookingsForQuote(
                                  quoteId,
                                  maxAttempts: 6,
                              ...
                              if (polled != null) {
                                ...
                                await _showBookingCreatedBottomSheet(
                                    _bookingId!, thread, null, null, null);
                                return;
                              }
                              // still not found: inform user
                              AppNotification.showInfo(context,
                                  'Payment received — awaiting server confirmation. Contact support if this takes too long.');
                            } catch (e) {
                              if (kDebugMode)
                                debugPrint('Retry poll error: $e');
                            }
                          },
                  ...
                  Expanded(
                      child: OutlinedButton(
                          onPressed: () async {
                            ...
                            try {
                              final uri = Uri.parse(
                                  'mailto:support@rijhub.com?subject=Payment%20confirmation%20help&body=Quote%20id:%20$quoteId');
      ...
      if (kDebugMode)
        debugPrint('showAwaitingServerConfirmationBottomSheet error: $e');
    }
  }
  ...
  bool _isQuotePayment() {
    try {
      ...
      final p = widget.payment;
      if (p is Map) {
        final bs = p['bookingSource'] ??
            (p['metadata'] is Map ? p['metadata']['bookingSource'] : null);
        if (bs != null && bs.toString().toLowerCase() == 'quote') return true;
        if (p['quoteId'] != null || p['quote'] != null) return true;
      }
      if (_initRequestPayload != null) {
        final bs2 = _initRequestPayload!['bookingSource'] ??
            (_initRequestPayload!['metadata'] is Map
                ? _initRequestPayload!['metadata']['bookingSource']
                : null);
  ...
  Future<bool> _acceptQuoteIfNeeded() async {
    try {
      final qid = widget.quote?['_id']?.toString() ??
          ...
          widget.quote?['quoteId']?.toString() ??
          _quoteId;
      if (qid == null || qid.isEmpty) return false;
      // derive jobId from payloads
      String? jobId;
      try {
        jobId = widget.payment['jobId']?.toString() ??
            widget.booking?['jobId']?.toString() ??
            widget.quote?['jobId']?.toString();
        if (jobId == null) {
  ...
  Future<Map<String, dynamic>?> _pollBookingStatus(String bookingId,
      {int maxAttempts = 7,
      Duration initialDelay = const Duration(seconds: 1)}) async {
    ...
    try {
      ...
      if (token != null && token.isNotEmpty)
        headers['Authorization'] = 'Bearer $token';
      final uri = Uri.parse(
          '${_normalizeBaseUrl(API_BASE_URL)}/api/bookings/$bookingId');
      ...
      while (attempt < maxAttempts) {
        ...
        try {
          ...
          if (kDebugMode)
            debugPrint(
                '_pollBookingStatus attempt $attempt -> ${resp.statusCode} ${resp.body}');
          if (resp.statusCode >= 200 &&
              resp.statusCode < 300 &&
              resp.body.isNotEmpty) {
            final decoded = jsonDecode(resp.body);
            ...
            if (data is Map) {
              ...
              final status = (data['status'] ?? data['booking']?['status'])
                      ?.toString()
                      .toLowerCase() ??
                  '';
              final paymentStatus =
              ...
              if (kDebugMode)
                debugPrint(
                    '_pollBookingStatus got status=$status paymentStatus=$paymentStatus');
              if (status.contains('await') ||
                  status == 'accepted' ||
                  paymentStatus == 'paid' ||
                  status == 'in-progress' ||
                  status == 'completed') {
  ...
  Future<String?> _fetchThreadIdForBooking(String bookingId) async {
    try {
      ...
      if (token != null && token.isNotEmpty)
        headers['Authorization'] = 'Bearer $token';
      final uri = Uri.parse(
          '${_normalizeBaseUrl(API_BASE_URL)}/api/chat/booking/$bookingId');
      final resp = await http
          .get(uri, headers: headers)
          .timeout(const Duration(seconds: 8));
      if (kDebugMode)
        debugPrint(
            'fetchThreadIdForBooking -> ${resp.statusCode} ${resp.body}');
      if (resp.statusCode >= 200 &&
          resp.statusCode < 300 &&
          resp.body.isNotEmpty) {
        final decoded = jsonDecode(resp.body);
  ...
  Future<void> _showBookingCreatedBottomSheet(
    ...
    try {
                ...
                ElevatedButton(
                    onPressed: () async {
                      ...
                      try {
                        ...
                        await rootNav.pushReplacement(MaterialPageRoute(
      ...
      if (kDebugMode) debugPrint('showBookingCreatedBottomSheet error: $e');
    }
  }

  Future<void> _showBookingFailedBottomSheet(
      String? jobTitle, String? bookingPrice, String? bookingDateTime) async {
    if (!mounted) return;
    try {
      await showModalBottomSheet<void>(
        context: context,
            ...
            child: Container(
              ...
              child: Column(mainAxisSize: MainAxisSize.min, children: [
                Text('Booking not created',
                    style: FlutterFlowTheme.of(ctx).titleLarge.copyWith(
                        fontWeight: FontWeight.bold,
                        color: Colors.red.shade700)),
                const SizedBox(height: 8),
                Text(
                    'We verified your payment but could not create the booking automatically. You can retry or contact support.',
                    textAlign: TextAlign.center,
                    style: FlutterFlowTheme.of(ctx).bodySmall),
                const SizedBox(height: 12),
                Row(children: [
      ...
      if (kDebugMode) debugPrint('showBookingFailedBottomSheet error: $e');
    }
  }

  Future<dynamic> _fetchQuoteById(
    ...
    try {
      ...
      if (jobId != null && jobId.isNotEmpty) {
        try {
          final uri = Uri.parse(
              '${_normalizeBaseUrl(API_BASE_URL)}/api/jobs/$jobId/quotes/$id');
          final resp = await http
              .get(uri, headers: headers)
              .timeout(const Duration(seconds: 8));
          if (kDebugMode)
            debugPrint(
                'Fetch job-scoped quote $id -> ${resp.statusCode} ${resp.body}');
          if (resp.statusCode >= 200 &&
              resp.statusCode < 300 &&
              resp.body.isNotEmpty) {
            final decoded = jsonDecode(resp.body);
          ...
          if (kDebugMode) debugPrint('Job-scoped fetch quote error: $e');
        }
      }
      // fallback to generic
      try {
        final uri =
            Uri.parse('${_normalizeBaseUrl(API_BASE_URL)}/api/quotes/$id');
        final resp = await http
            .get(uri, headers: headers)
            .timeout(const Duration(seconds: 8));
        if (kDebugMode)
          debugPrint(
              'Fetch generic quote $id -> ${resp.statusCode} ${resp.body}');
        if (resp.statusCode >= 200 &&
            resp.statusCode < 300 &&
            resp.body.isNotEmpty) {
          final decoded = jsonDecode(resp.body);
        ...
        if (kDebugMode) debugPrint('Generic fetch quote error: $e');
      }
    } catch (e) {
      if (kDebugMode) debugPrint('Fetch quote by id unexpected error: $e');
    }
    return null;
  }
  ...
  Widget build(BuildContext context) {
    ...
    final List<dynamic> selectedServices = (widget.payment['metadata'] is Map
            ? (widget.payment['metadata']['services'] ??
                widget.payment['services'])
            : (widget.payment['services'] ?? [])) as List<dynamic>? ??
    ...
    try {
      if (selectedServices.isNotEmpty) {
        ...
        serviceTitle = widget.booking?['service'] ??
            widget.quote?['title'] ??
            widget.payment['service'] ??
            'Service';
      }
    } catch (_) {
      serviceTitle = widget.booking?['service'] ??
    ...
    final serviceSubtitle = widget.booking?['serviceDetail'] ??
    ...
    return WillPopScope(
      ...
      child: Scaffold(
        ...
        body: Stack(children: [
          ...
          if ((_state == _PaymentState.verifying) ||
              (_loading && _blocking)) ...[
            Positioned.fill(
              child: Container(
