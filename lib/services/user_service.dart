...
class UserService {
      ...
      try {
        ...
        if (resp.statusCode >= 200 && resp.statusCode < 300) {
          ...
          if (decoded is Map) {
            ...
            if (decoded['data'] is Map) candidate = Map<String, dynamic>.from(decoded['data']);
            else if (decoded['payload'] is Map) candidate = Map<String, dynamic>.from(decoded['payload']);
            else if (decoded['user'] is Map) candidate = Map<String, dynamic>.from(decoded['user']);
            else if (decoded['profile'] is Map) candidate = Map<String, dynamic>.from(decoded['profile']);
            else candidate = Map<String, dynamic>.from(decoded);
            ...
            try {
              ...
              if (candidate['location'] != null && addr == null) addr = candidate['location']?.toString();
              if (candidate['latitude'] != null && lat == null) lat = (candidate['latitude'] is num) ? candidate['latitude'].toDouble() : double.tryParse(candidate['latitude'].toString());
              if (candidate['longitude'] != null && lon == null) lon = (candidate['longitude'] is num) ? candidate['longitude'].toDouble() : double.tryParse(candidate['longitude'].toString());
  ...
  static Future<Map<String, dynamic>> updateProfile(
    Map<String, String> fields, {String? imagePath, Uint8List? imageBytes, String? imageFilename, bool forceUserPath = false}
  ) async {
    final token = await TokenStorage.getToken();
    ...
    http.Response resp;
    if ((imageBytes != null && imageBytes.isNotEmpty) || (imagePath != null && imagePath.isNotEmpty && !imagePath.startsWith(RegExp(r'https?://')))) {
      ...
      if (imageBytes != null && imageBytes.isNotEmpty) {
        ...
        final multipartFile = http.MultipartFile.fromBytes('profileImage', imageBytes, filename: filename);
        req.files.add(multipartFile);
      } else if (imagePath != null && imagePath.isNotEmpty && !imagePath.startsWith(RegExp(r'https?://'))) {
        final file = File(imagePath);
        if (!await file.exists()) throw Exception('Profile image file not found');
        final stream = http.ByteStream(file.openRead());
        final length = await file.length();
        final multipartFile = http.MultipartFile('profileImage', stream, length, filename: file.path.split(Platform.pathSeparator).last);
        req.files.add(multipartFile);
      }
      ...
      resp = await http.put(uri, headers: headers, body: jsonEncode(fields)).timeout(const Duration(seconds: 30));

      // If server refuses JSON with a multipart requirement, retry using multipart/form-data (fields-only)
      if (resp.statusCode == 406 || (resp.body != null && resp.body.toLowerCase().contains('not multipart'))) {
        try {
          ...
          final streamedRetry = await retryReq.send().timeout(const Duration(seconds: 60));
    ...
    if (resp.statusCode >= 200 && resp.statusCode < 300) {
      ...
      try {
        Map<String, dynamic>? candidate;
        if (decoded is Map && decoded['data'] is Map) candidate = Map<String, dynamic>.from(decoded['data']);
  ...
  static String? _extractRole(Map<String, dynamic>? profile) {
    ...
    try {
      ...
      for (final c in candidates) {
        ...
        if (s.contains('customer') || s.contains('client') || s.contains('user')) return 'customer';
  ...
  static bool? _extractKyc(Map<String, dynamic>? profile) {
    if (profile == null) return null;
    try {
      final candidates = [profile['kycVerified'], profile['kyc']?['verified'], profile['kycVerifiedFlag'], profile['isVerified']];
      for (final c in candidates) {
        if (c == null) continue;
  ...
  static Future<Map<String, dynamic>> getCanonicalLocation() async {
    try {
      ...
      if ((cached['address'] as String?)?.isNotEmpty == true || cached['latitude'] != null || cached['longitude'] != null) {
        return cached;
      }
      final profile = await getProfile();
      if (profile == null) return {'address': null, 'latitude': null, 'longitude': null};
      String? addr;
      double? lat;
      double? lon;
      try {
        ...
        if (profile['location'] != null && addr == null) addr = profile['location']?.toString();
        if (profile['latitude'] != null && lat == null) lat = (profile['latitude'] is num) ? profile['latitude'].toDouble() : double.tryParse(profile['latitude'].toString());
        if (profile['longitude'] != null && lon == null) lon = (profile['longitude'] is num) ? profile['longitude'].toDouble() : double.tryParse(profile['longitude'].toString());
      } catch (_) {}
      // cache for later
      try { await TokenStorage.saveLocation(address: addr, latitude: lat, longitude: lon); } catch (_) {}
