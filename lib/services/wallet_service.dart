...
class WalletServiceException implements Exception {
  ...
  @override
  String toString() => 'WalletServiceException: $message ${statusCode != null ? '(status $statusCode)' : ''}';
}
  ...
  static Future<List<Map<String, dynamic>>> fetchTransactions({
    ...
    for (final ep in endpoints) {
      try {
        ...
        final uri = Uri.parse(ep).replace(queryParameters: { 'page': page.toString(), 'limit': limit.toString() });
        ...
        if (resp.statusCode >= 200 && resp.statusCode < 300 && resp.body.isNotEmpty) {
          ...
          if (list is List) {
            final allTx = List<Map<String, dynamic>>.from(list.map((e) => e is Map ? Map<String, dynamic>.from(e) : {'raw': e}));
            return allTx;
          }
        }
      } catch (_) {
  ...
  static Future<Map<String, dynamic>> savePayoutDetails({
    ...
    if (!(resp.statusCode >= 200 && resp.statusCode < 300 && resp.body.isNotEmpty)) {
      try {
        ...
        final streamed = await client.send(mpReq).timeout(const Duration(seconds: 15));
