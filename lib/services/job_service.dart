...
class JobService {
  static Future<Map<String, dynamic>> createJob(Map<String, dynamic> payload) async {
    ...
    final res = await ApiClient.post(url, headers: {'Content-Type': 'application/json'}, body: jsonEncode(payload));
    ...
    final userMessage = res['userMessage'] as String? ?? 'Failed to create job.';
  ...
  static Future<List<Map<String, dynamic>>> getJobCategories({int page = 1, int limit = 100}) async {
    ...
    final resp = await http.get(uri, headers: headers).timeout(Duration(seconds: 15));
    if (resp.statusCode >= 200 && resp.statusCode < 300) {
      final body = resp.body.isNotEmpty ? jsonDecode(resp.body) : null;
      if (body == null) return [];
      // body may be { success: true, data: [...] }
      if (body is Map && body['data'] is List) return List<Map<String, dynamic>>.from(body['data'].map((e) => Map<String, dynamic>.from(e)));
      if (body is List) return List<Map<String, dynamic>>.from(body.map((e) => Map<String, dynamic>.from(e)));
      return [];
    }
    throw Exception('Failed to fetch job categories: ${resp.statusCode} ${resp.body}');
  }
  ...
  static Future<List<Map<String, dynamic>>> getAllJobs() async {
    ...
    if (resp.statusCode >= 200 && resp.statusCode < 300) {
      ...
      if (body is Map && body['items'] is List) return List<Map<String, dynamic>>.from(body['items'].map((e) => Map<String, dynamic>.from(e)));
      return [];
    }
  ...
  static Future<List<Map<String, dynamic>>> getJobs({int page = 1, int limit = 12, String? query}) async {
    final token = await TokenStorage.getToken();
    final headers = <String, String>{'Content-Type': 'application/json'};
    if (token != null) headers['Authorization'] = 'Bearer $token';
    var uri = Uri.parse('$API_BASE_URL/api/jobs');
    ...
    if (resp.statusCode >= 200 && resp.statusCode < 300) {
      ...
      if (body is Map) {
        ...
        for (final k in possibleLists) {
          if (body[k] is List) return List<Map<String, dynamic>>.from(body[k].map((e) => Map<String, dynamic>.from(e)));
        }
      }
      return [];
    }
    throw Exception('Failed to fetch paginated jobs: ${resp.statusCode} ${resp.body}');
  }

  /// Fetch open jobs from the server. If `params` is provided it's appended
  /// as query parameters. This returns decoded JSON as a List if possible.
  static Future<List<Map<String, dynamic>>> getUserJobs({Map<String, String>? params}) async {
    final token = await TokenStorage.getToken();
    final headers = <String, String>{'Content-Type': 'application/json'};
    if (token != null) headers['Authorization'] = 'Bearer $token';
  ...
  static Future<List<Map<String, dynamic>>> getMyJobs() async {
    ...
    final myId = (profile['id'] ?? profile['_id'] ?? profile['userId'])?.toString();
    // print(myId);
    // print(await getJobs());
    try {
    ...
    try {
      return all.where((j) {
        ...
        if (cid is Map) return (cid['_id']?.toString() ?? cid['id']?.toString() ?? '') == myId;
  ...
  static Future<Map<String, dynamic>> updateJob(String id, Map<String, dynamic> payload) async {
    if (id.isEmpty) throw Exception('Job id required');
    final token = await TokenStorage.getToken();
    final headers = <String, String>{'Content-Type': 'application/json'};
    if (token != null) headers['Authorization'] = 'Bearer $token';
    final uri = Uri.parse('$API_BASE_URL/api/jobs/$id');
    final resp = await http.patch(uri, headers: headers, body: jsonEncode(payload)).timeout(Duration(seconds: 15));
    if (resp.statusCode >= 200 && resp.statusCode < 300) {
