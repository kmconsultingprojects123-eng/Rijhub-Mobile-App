...
T createModel<T extends FlutterFlowModel>(
  ...
  // Attempt to read a provided model from the widget tree. In some cases
  // there won't be a Provider for the page model (e.g. when the page is
  // being built standalone). context.read<T?>() throws if no provider is
  // present, which can prevent _model from being initialized and cause
  // LateInitializationError later. We therefore guard the call and fall
  // back to the default builder when needed.
  T? model;
  try {
    // Using context.read<T?>() may throw ProviderNotFoundException; catch it.
    model = context.read<T?>();
  } catch (_) {
    model = null;
  }

  final result = model ?? defaultBuilder();
  result._init(context);
  return result;
}
