...
export 'nav/nav.dart';
export 'nav/ff_navigation_adapters.dart'; // Also export the adapter directly for files that import flutter_flow_util
...
void showSnackbar(
  ...
  // Use overlay-based snack dialog for consistency with AwesomeDialog helpers.
  // Loading state isn't represented in the overlay; if needed we can extend the helper.
  showAppSnackDialog(context, message);
}
...
// AppBar color helpers: ensure AppBar background is white in light mode and dark in dark mode,
// and ensure title/icon colors contrast accordingly.
...
/// Centralized debug-only logger. Use ffLog(...) instead of direct debugPrint so
/// we can globally silence logs if needed.
void ffLog(Object? message, {String? tag}) {
  try {
    if (kDebugMode) {
      final out = (tag != null) ? '[$tag] $message' : message?.toString() ?? '';
      debugPrint(out);
    }
  } catch (_) {}
}
