...
- `GET /api/users/:id/full` — (protected) get full aggregated profile for a user
    - **Purpose:** Return everything related to a user across collections so admin or the user can inspect transactions, wallet, jobs, bookings, quotes, KYC records, device tokens, notifications, reviews, applications, and chats in a single response.
    - **Headers:** `Authorization: Bearer <TOKEN>` (required). Only admins or the user themself may access this endpoint.
    - **Params:** `id` — target user ObjectId (24 hex chars) — the user to fetch.
    - **Query (optional):**
        - `limit` — number per-collection page (default 50, max 200).
        - `transactionsCursor` — ISO timestamp cursor to paginate transactions (createdAt &lt; cursor).
        - `bookingsCursor` — ISO timestamp cursor to paginate bookings (createdAt &lt; cursor).
        - `jobsCursor` — ISO timestamp cursor to paginate jobs (createdAt &lt; cursor).
        - `quotesCursor` — ISO timestamp cursor to paginate quotes (createdAt &lt; cursor).
        - `notificationsCursor` — ISO timestamp cursor to paginate notifications (createdAt &lt; cursor).
        - `reviewsCursor` — ISO timestamp cursor to paginate reviews (createdAt &lt; cursor).
        - `applicationsCursor` — ISO timestamp cursor to paginate applications (createdAt &lt; cursor).
        - `chatsCursor` — ISO timestamp cursor to paginate chats (uses `updatedAt` for ordering).
    - **Response (200):** `{ success: true, data: { user, artisanProfile, wallet, transactions:[], bookings:[], jobs:[], quotes:[], kycs:[], deviceTokens:[], notifications:[], reviews:[], applications:[], chats:[], cursors: { transactions, bookings, jobs, quotes, notifications, reviews, applications, chats }, counts: { ... } } }`
        - Each collection is returned as a paginated page (up to `limit` items). The `cursors` object contains ISO timestamps to use for the next page for each collection. Use the cursor value as the corresponding `*Cursor` query param to fetch the next page.
    - **Populates:** the endpoint includes lightweight populates for common references:
        - `Job.clientId` populated with `name,email,profileImage`.
        - `Booking.artisanId` and `Booking.customerId` populated with `name,email,profileImage`.
        - `Transaction.payerId` and `Transaction.payeeId` populated with `name,email`.
        - `Quote.artisanId` and `Quote.customerId` populated with `name,email,profileImage`.
    - **Notes:**
        - Designed for admin/diagnostic usage. If you need full counts (total items across collections), request them explicitly as they may add extra latency.
        - Cursors use ISO timestamps from the item's `createdAt` (or `updatedAt` for chats). The API returns a `nextCursor` per collection when more items exist.

    - **Example (Flutter):**

```
// Fetch first page
final res = await http.get(Uri.parse('http://localhost:5000/api/users/<USER_ID>/full?limit=50'), headers: {'Authorization': 'Bearer $token'});
final body = jsonDecode(res.body);
final jobs = body['data']['jobs'];
final jobsNextCursor = body['data']['cursors']['jobs'];

// Fetch next page of jobs
if (jobsNextCursor != null) {
  final res2 = await http.get(Uri.parse('http://localhost:5000/api/users/<USER_ID>/full?limit=50&jobsCursor=$jobsNextCursor'), headers: {'Authorization': 'Bearer $token'});
}
```
...
- **Category Integration:** Artisans can now be organized using hierarchical job categories:
    ...
    - See [Job Categories](#job-categories) section for category management endpoints
...
- `POST /api/artisans` — (protected) create artisan profile
  ...
  **Option 1: JSON (application/json)** — Portfolio images must be uploaded to Cloudinary beforehand, and URLs included in the request:
  - Body (JSON):
  - `trade`: array of strings (required) — e.g. `["plumber", "electrician"]` (legacy field, kept for backward compatibility)
  - `categories`: array of ObjectId strings (optional) — e.g. `["507f1f77bcf86cd799439011", "507f191e810c19729de860ea"]` — references to JobCategory documents. Use this for hierarchical category organization.
...
- `PUT /api/artisans/me` — (protected) update authenticated user's artisan profile
  ...
  **Option 1: JSON (application/json):**
  ...
  - `trade`: array of strings
  - `categories`: array of ObjectId strings — references to JobCategory documents
  - `experience`: number
...
1. **Customer books artisan**: `POST /api/bookings/hire`
    - Booking created with `status: 'pending'`, `paymentStatus: 'unpaid'`
    - Paystack payment initialized
    - Customer completes payment

2. **Payment confirmed**: Webhook/admin calls `POST /api/bookings/:id/confirm-payment`
...
3. **Artisan responds** (within 24 hours):
...
4. **Auto-rejection** (if artisan doesn't respond):
...
5. **Work completion**: Customer calls `POST /api/bookings/:id/complete`
...
**Artisan Profile Progress (client usage)**

- **Overview:** The API now includes a `profileProgress` numeric field on artisan responses returned by `GET /api/artisans` (list/search) and `GET /api/artisans/:id`. This helps mobile/web clients show a visual progress indicator for artisan onboarding.
- **Meaning:** value is one of: `0`, `40`, `80`, or `100` (integers).
	- `0` — no KYC and no profile content detected
	- `40` — either KYC submitted OR profile content present
	- `80` — both KYC submitted and profile content present
	- `100` — artisan marked `verified` by admin (verified overrides other values)

- **Where it appears:** In the JSON returned by `GET /api/artisans`, `GET /api/artisans?*` (search/list), and `GET /api/artisans/:id` under each artisan object as `profileProgress`.

- **Flutter example (read and show percent):**

```dart
// After fetching artisans list
final body = jsonDecode(response.body);
final artisans = body['data'] as List<dynamic>;
for (var a in artisans) {
	final int progress = (a['profileProgress'] ?? 0) as int;
	// Use a linear progress indicator (0..1.0)
	final double pct = (progress / 100.0).clamp(0.0, 1.0);
	// show in UI: e.g. LinearProgressIndicator(value: pct)
}
```

- **Notes & best practices:**
    - Treat `profileProgress` as informational only; do not rely on it for access control. The authoritative verification flag is `verified` on the artisan (and `isVerified`/`kycVerified` on the user record).
    - The value is additive: `profileProgress = profileBase (0|40) + kyc (0|40)`, then `verified` forces `100`.
    - Existing clients that ignore unknown fields will continue to work; adding this field is non-breaking.
      title: { type: 'string' },
...
Additional per-feature docs

- Paystack helpers: [docs/paystack_helpers.md](docs/paystack_helpers.md)
- Device tokens: [docs/device_tokens.md](docs/device_tokens.md)
- Account deletion: [docs/account_deletion.md](docs/account_deletion.md)
- Artisan profile progress: [docs/artisan_profile_progress.md](docs/artisan_profile_progress.md)
- Job public id (short identifier): [docs/job_public_id.md](docs/job_public_id.md)
- Job publicId backfill instructions: [docs/job_publicid_backfill.md](docs/job_publicid_backfill.md)
- Job subcategories: [docs/job_subcategories.md](docs/job_subcategories.md)
- Registration OTP (email verification): [docs/registration_otp.md](docs/registration_otp.md)
- Full customer profile (aggregated): [docs/full_customer_profile.md](docs/full_customer_profile.md)
